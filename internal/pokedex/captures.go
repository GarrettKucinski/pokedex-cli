...
func (p *Pokemon) AttemptCapture() (captureSuccess bool) {
	captureChance := float64(rand.Intn(p.BaseExperience))
	captureThreshold := float64(p.BaseExperience) * float64(0.70)

	fmt.Printf("Threw a Pokeball at %s\n", p.Name)

	if captureChance > captureThreshold {
		return true
	}
